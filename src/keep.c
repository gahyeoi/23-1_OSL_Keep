...
int untrack(char * path) 
{
    ...
    // track하고 있지 않던 것을 untrack하려고 하면 오류를 보여줘야 함 
    char line[4096] ;
...
int store(char * note)
{
    ...
    // .keep 아래에 version directory 생성 
    mkdir(version_path, 0755) ;

    // .keep/tracking-files 생성 후 tracking-files에 대한 내용 넣기 
...
{   
    // 1. .keep의 내부의 있는 것을 version으로 다 넣기 
    //      1-1) main에 있는 file들을 전부 삭제
    //      1-2) ./keep/version에 있는 파일을 전부 복붙해오기.ㅠㅠㅠㅠ 
    
    struct dirent *entry;

    // version으로 부른 것의 trackingfile과
    char version_path[PATH_MAX];
    strcpy(version_path, ".keep/");
    strcpy(version_path + 6, version);
    strcpy(version_path + 7, "/tracking-files");

    // 현재 있는 trackingfile과 최근의 trackingfile이 서로 같은지 비교..
    FILE *k_version = fopen(".keep/latest-version", "r");
    if (k_version == 0x0)
    {
        exit(1);
